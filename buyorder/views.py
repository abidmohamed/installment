...
from billingbuyorder.models import BuyOrderBilling, BillBuyOrderItem
from buyorder.forms import BuyOrderForm, BuyOrderItemFormset
from buyorder.models import BuyOrderItem, BuyOrder
from product.models import Product, ProductColor, ProductType
    ...
    if request.method == 'POST':
        ...
        if buyorderform.is_valid():
            ...
            for index, item in enumerate(buyorder.items.all()):
                # print(index, item)
                print(prices[index], quantities[index], colors[index], types[index])
                # get the price and value of each element
                # Saving the orderitem
                item.price = prices[index]
                item.quantity = quantities[index]
                if colors[index] != "None":
                    item.color = ProductColor.objects.get(id=colors[index])
                if types[index] != "None":
...
def buyorderorder_list_by_supplier(request, pk):
    ...
    if request.method == 'POST':
        ...
        if len(chosenorders) != 0:
            ...
            return redirect(f'../../billingbuyorder/bill_pdf/{pk}')
